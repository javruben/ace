...
var TextInput = function(parentNode, host) {
    ...
    if (true /* iOS */) {
        style.left = "0px";//"-10000px";
        style.top = "0px";//"-10000px";
        style.zIndex = -1;
        style.opacity = 0;
        style.width = "10px";
        style.height = "30px";
        
        var fChangeCursor = function(e){
            var pos = host.renderer.textToScreenCoordinates(e.data.row, e.data.column);
            var epos = parentNode.getBoundingClientRect();
            style.left = (pos.pageX - epos.left - 6) + "px";
            style.top = (pos.pageY - epos.top + 52) + "px";
        };
        
        host.addEventListener("changeDocument", function(e){
            if (e.oldDoc)
                e.oldDoc.getSelection().removeEventListener("changeCursor", fChangeCursor);
            e.doc.getSelection().addEventListener("changeCursor", fChangeCursor);
        });
    }
